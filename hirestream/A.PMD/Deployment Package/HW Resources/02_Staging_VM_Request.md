...
### 3.6 Build & System Tools (Additional)

| # | Software | Install Command | Purpose |
|:---|:---|:---|:---|
| 15 | **Python 3** | `sudo apt install -y python3` | Required by node-gyp for native module builds (bcrypt, bufferutil) |
| 16 | **logrotate** | `sudo apt install -y logrotate` | Log file rotation for application and Nginx logs |
| 17 | **openssl** | Pre-installed on Ubuntu — verify with `openssl version` | Secret generation for session keys, tokens |

### 3.7 Fonts (for PDF rendering and bilingual support)
