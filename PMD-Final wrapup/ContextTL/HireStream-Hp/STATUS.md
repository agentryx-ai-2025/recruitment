...
- **v0.6.0 — Hindi pass, increment 1: the simplified candidate dashboard.** Added a `simpleDash` namespace to `en.json` + `hi.json` (Fable-drafted Hindi) and converted `candidate-dashboard-simple.tsx` to `t()`. Verified live: language=hi renders the dashboard in Devanagari; the header हिं toggle switches it. **First-draft Hindi — needs a native-speaker review before go-live.** Remaining flows (`/apply`, `/apply/pro`, `/start`, `/documents`, QuestionShell) still English — same i18n pattern to apply.
